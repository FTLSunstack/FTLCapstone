...
function LoginForm({language}) {
    ...
    const [forgotPasswordTxt, setforgotPasswordTxt] = useState( "¿Olvidó su contraseña?" );
    ...
    if (modalOpen) {
        modalElement = <PasswordModal email={email} setEmail={setEmail} closePasswordModal={closePasswordModal} language={language} />;
    }
        ...
        <>
            <div className="LoginForm bg-white bg-center flex flex-col items-center w-100 h-140 rounded-md "> 
                ...
                <form onSubmit={userLogin} className=" w-full flex flex-col items-center">
                    <LoginInput username={username} setUsername={setUsername} password={password} setPassword={setPassword} language={language} ></LoginInput>
                    <div className= " w-2/3 flex flex-col items-center justify-center align-center">
                        <p onClick = {openPasswordModal} className="mt-4 ml-1 text-center text-indigo-600 hover:text-purple-800 transition ease-in-out hover:cursor-pointer"> {forgotPasswordTxt}</p>
                        <button type="submit" className="mt-2 mb-4 py-2 px-9 text-center font-semibold rounded-lg shadow-md text-white bg-indigo-700 hover:bg-blue-800 cursor-pointer"> {loginButtonTxt}</button>
                    </div>
                </form>
                <div className= "flex flex-row">
                    <p className="text-xs text-gray-500"> {AccountTxt} </p>
                    <p onClick={handleSignUp} className="ml-3 text-xs text-gray-500 underline hover:text-indigo-500 hover:cursor-pointer"> {signupTxt}</p>
                </div>
